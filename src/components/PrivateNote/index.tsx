  ...
  return (
    <div
      className={cn(
        ...
        'p-3 mb-3 cursor-pointer transition-all hover:shadow-lg hover:scale-[1.01]',
    ...
    >
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            ...
            <div
              ...
              style={{ fontFamily: "'Kalam', cursive" }}
            >
              {truncatedText || t('Click to add a note...')}
            </div>
          )}
        </div>
        <button
      ...
      {note?.noteEventId && (
        <div className="text-xs text-yellow-700 dark:text-yellow-800 mt-2 flex items-center gap-1">
          <StickyNote className="size-3" />
          <span>{t('Pinned note attached')}</span>
        </div>
      )}
    </div>
  )
