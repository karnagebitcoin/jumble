...
import AICommandList, { AICommandListHandle, AICommandListProps } from './AICommandList'

const suggestion = {
  items: async ({ query }: { query: string }) => {
    // Return the query as-is for the AI to process
    return [{ query }]
  },

  render: () => {
    let component: ReactRenderer<AICommandListHandle, AICommandListProps> | undefined
    let popup: Instance[] = []
